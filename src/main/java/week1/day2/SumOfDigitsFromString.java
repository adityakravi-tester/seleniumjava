package week1.day2;

public class SumOfDigitsFromString {
	
	/*
	 * Method 1
	 * Pseudo Code
	 
	 * Declare a String text with the following value
		String text = "Tes12Le79af65";
	   Declare a int variable sum
		int sum = 0;
	 * a) using replaceAll(), replace all the non-digits into ""
	 * b) Now, convert the String into array
	 * c) Iterate over the array and get each character
	 * d) Using Character.getNumericValue(), Change the char into int
	 * e) Add the values to sum & print
	 * 
	 */
	
	/*
	 * Method 2
	 * Pseudo Code
	 * Declare a String text with the following value
		String text = "Tes12Le79af65";
	   Declare a int variable sum
		int sum = 0;
	 * a) Iterate an  array over the String
	 * b) Get each character and check if it is a number using Character.isDigit()
	 * c) Now covert char to int using Character.getNumericValue() and add it to sum
	 * d) Now Print the value
	 */
	public static void main(String[] args) {
		new SumOfDigitsFromString().getSumOfStringConvertedDigits("Tes12Le79af65");
		new SumOfDigitsFromString().getSumOfStringConvertedDigits2("Tes12Le79af65");
	}
	
	private void getSumOfStringConvertedDigits(String str) {
		String changedString = str.replaceAll("[^\\d.]", "");
		char[] charArray = changedString.toCharArray();
		int sum = 0;
		for (int i = 0; i < charArray.length; i++) {
			int numericValue = Character.getNumericValue(charArray[i]);
			sum = sum + numericValue;
		}
		
		System.out.println("The sum of digits from string is: " + sum);
	}
	
	private void getSumOfStringConvertedDigits2(String str) {
		char[] charArray = str.toCharArray();
		int sum = 0;
		for (int i = 0; i < charArray.length; i++) {
			if(Character.isDigit(charArray[i])) {
				int numericValue = Character.getNumericValue(charArray[i]);
				sum = sum + numericValue;
			}
		}
		System.out.println("The sum of digits from string is: " + sum);
	}

}
